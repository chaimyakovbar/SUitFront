// S3 Assets URLs - all images now served from S3
const S3_BASE_URL = "https://ch-suits.s3.us-east-1.amazonaws.com";

// Colors
// import photo1 from "../assets/homeMenu/color/1.webp";
// import photo2 from "../assets/homeMenu/color/black.webp";
// import photo3 from "../assets/homeMenu/color/blueLight.webp";
// import photo4 from "../assets/homeMenu/color/brun.webp";
// import photo5 from "../assets/homeMenu/color/brownlight.webp";
// import photo6 from "../assets/homeMenu/color/burgundy.webp";
// import photo7 from "../assets/homeMenu/color/bezz.webp";
// import photo8 from "../assets/homeMenu/color/TR224-17 (NEVY).webp";
import photo1 from "../assets/homeMenu/color/TR224-6.webp";
import photo2 from "../assets/homeMenu/color/TR224-7.webp";
import photo3 from "../assets/homeMenu/color/TR224-10.webp";
import photo4 from "../assets/homeMenu/color/TR224-12.webp";
import photo5 from "../assets/homeMenu/color/TR224-14.webp";
import photo6 from "../assets/homeMenu/color/TR224-16.webp";
import photo7 from "../assets/homeMenu/color/TR224-17.webp";
import photo8 from "../assets/homeMenu/color/TR224-18.webp";
import photo9 from "../assets/homeMenu/color/TR224-19.webp";

// Color Inside Up
export const gold = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/gold.jpg`;
export const grey = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/grey.jpg`;
export const purple = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/purple.jpg`;
export const blueCube = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/blueCube.jpg`;
export const blueLight = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/blueLight.jpg`;
export const redPurple = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/redPurple.jpg`;
export const colorFull = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/colorFull.jpg`;
export const whiteBlue = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/whiteBlue.jpg`;
export const blueWithWhiteLines = `${S3_BASE_URL}/assets_V3/adds/colorInsideUp/blueWithWhiteLines.jpg`;

// Buttons
export const blackBlue = `${S3_BASE_URL}/assets_V3/adds/button/blackBlue.jpg`;
export const brownBlack = `${S3_BASE_URL}/assets_V3/adds/button/brownBlack.jpg`;
export const brownWhite = `${S3_BASE_URL}/assets_V3/adds/button/brownWhite.jpg`;
export const gold2 = `${S3_BASE_URL}/assets_V3/adds/button/gold.jpg`;
export const goldLight = `${S3_BASE_URL}/assets_V3/adds/button/goldLight.jpg`;
export const greyBlack = `${S3_BASE_URL}/assets_V3/adds/button/greyBlack.jpg`;
export const greyWhite = `${S3_BASE_URL}/assets_V3/adds/button/greyWhite.jpg`;
export const silver = `${S3_BASE_URL}/assets_V3/adds/button/silver.jpg`;
export const silverLight = `${S3_BASE_URL}/assets_V3/adds/button/silverLight.jpg`;
export const white = `${S3_BASE_URL}/assets_V3/adds/button/white.jpg`;

// Poshet
import poset2 from "../assets/homeMenu/poshet/black2.webp";
import poset4 from "../assets/homeMenu/poshet/blackWithDots.webp";
import poset6 from "../assets/homeMenu/poshet/blueLight.webp";
import poset7 from "../assets/homeMenu/poshet/blueLightWithDots.webp";
import poset9 from "../assets/homeMenu/poshet/blueNevey2.webp";
import poset10 from "../assets/homeMenu/poshet/blueWithBlueLightLins.webp";
import poset11 from "../assets/homeMenu/poshet/blueWithBlueLightLins2.webp";
import poset12 from "../assets/homeMenu/poshet/blueWithDots.webp";
import poset13 from "../assets/homeMenu/poshet/blueWithDots2.webp";
import poset14 from "../assets/homeMenu/poshet/blueWithLines.webp";
import poset15 from "../assets/homeMenu/poshet/brown.webp";
import poset16 from "../assets/homeMenu/poshet/brownWithDots.webp";
import poset17 from "../assets/homeMenu/poshet/gold.webp";
import poset18 from "../assets/homeMenu/poshet/goldWhite.webp";
import poset19 from "../assets/homeMenu/poshet/grey.webp";
import poset20 from "../assets/homeMenu/poshet/peenk.webp";
import poset21 from "../assets/homeMenu/poshet/prupleWithDots.webp";
import poset22 from "../assets/homeMenu/poshet/purple.webp";
import poset23 from "../assets/homeMenu/poshet/purpleLight.webp";
import poset25 from "../assets/homeMenu/poshet/red2.webp";
import poset26 from "../assets/homeMenu/poshet/redLight.webp";
import poset28 from "../assets/homeMenu/poshet/redWithDots2.webp";
import poset29 from "../assets/homeMenu/poshet/redWithines.webp";
import poset30 from "../assets/homeMenu/poshet/white.webp";

// Suits - Updated to use .webp format for better performance
export const Suit1 = `${S3_BASE_URL}/assets_V3/suits/1.webp`;
export const Suit2 = `${S3_BASE_URL}/assets_V3/suits/2.webp`;
export const Suit3 = `${S3_BASE_URL}/assets_V3/suits/3.webp`;
export const Suit4 = `${S3_BASE_URL}/assets_V3/suits/4.webp`;
export const Suit5 = `${S3_BASE_URL}/assets_V3/suits/5.webp`;
export const Suit6 = `${S3_BASE_URL}/assets_V3/suits/6.webp`;
export const Suit7 = `${S3_BASE_URL}/assets_V3/suits/7.webp`;
export const Suit8 = `${S3_BASE_URL}/assets_V3/suits/8.webp`;
export const Suit9 = `${S3_BASE_URL}/assets_V3/suits/9.webp`;
export const Suit10 = `${S3_BASE_URL}/assets_V3/suits/10.webp`;
export const Suit11 = `${S3_BASE_URL}/assets_V3/suits/11.webp`;
export const Suit12 = `${S3_BASE_URL}/assets_V3/suits/12.webp`;
export const Suit13 = `${S3_BASE_URL}/assets_V3/suits/13.webp`;
export const Suit14 = `${S3_BASE_URL}/assets_V3/suits/14.webp`;
export const Suit15 = `${S3_BASE_URL}/assets_V3/suits/15.webp`;
export const Suit16 = `${S3_BASE_URL}/assets_V3/suits/16.webp`;
export const Suit17 = `${S3_BASE_URL}/assets_V3/suits/17.webp`;
export const Suit18 = `${S3_BASE_URL}/assets_V3/suits/18.webp`;
export const Suit19 = `${S3_BASE_URL}/assets_V3/suits/19.webp`;
export const Suit20 = `${S3_BASE_URL}/assets_V3/suits/20.webp`;
export const Suit21 = `${S3_BASE_URL}/assets_V3/suits/21.webp`;

import size1 from "../assets/sizes/1.webp";
import size2 from "../assets/sizes/2.webp";
import size3 from "../assets/sizes/3.webp";
import size4 from "../assets/sizes/4.webp";
import size5 from "../assets/sizes/5.webp";
import size6 from "../assets/sizes/6.webp";
import size7 from "../assets/sizes/7.webp";
import size8 from "../assets/sizes/8.webp";
import size9 from "../assets/sizes/9.webp";
import size10 from "../assets/sizes/10.webp";
import size11 from "../assets/sizes/11.webp";
import size12 from "../assets/sizes/12.webp";
import size13 from "../assets/sizes/13.webp";
import size14 from "../assets/sizes/14.webp";
import size15 from "../assets/sizes/15.webp";
import size16 from "../assets/sizes/16.webp";

// Videos
export const Chest = `${S3_BASE_URL}/assets_V3/videoExplaine/Chest.mp4`;
export const Waist = `${S3_BASE_URL}/assets_V3/videoExplaine/Waist.mp4`;
export const Seat = `${S3_BASE_URL}/assets_V3/videoExplaine/Seat.mp4`;
export const JacketLength = `${S3_BASE_URL}/assets_V3/videoExplaine/Jacket Length.mp4`;
export const FrontWidth = `${S3_BASE_URL}/assets_V3/videoExplaine/Front Width.mp4`;
export const RearWidth = `${S3_BASE_URL}/assets_V3/videoExplaine/Rear Width.mp4`;
// export const Armhole = `${S3_BASE_URL}/assets_V3/videoExplaine/Armhole.mp4`;
export const Biceps = `${S3_BASE_URL}/assets_V3/videoExplaine/Biceps.mp4`;
export const Shoulder = `${S3_BASE_URL}/assets_V3/videoExplaine/Shoulder.mp4`;
export const SleeveLengthFront = `${S3_BASE_URL}/assets_V3/videoExplaine/Sleeve Length.mp4`;
export const TrousersLengthBack = `${S3_BASE_URL}/assets_V3/videoExplaine/Trousers Length back.mp4`;
export const TrousersLengthFront = `${S3_BASE_URL}/assets_V3/videoExplaine/Trousers Length front.mp4`;
export const Waistband = `${S3_BASE_URL}/assets_V3/videoExplaine/Waistband.mp4`;
export const StrideLength = `${S3_BASE_URL}/assets_V3/videoExplaine/Stride Length.mp4`;
export const Thigh = `${S3_BASE_URL}/assets_V3/videoExplaine/Thigh.mp4`;
// export const Knee = `${S3_BASE_URL}/assets_V3/videoExplaine/Knee.mp4`;
export const Ankles = `${S3_BASE_URL}/assets_V3/videoExplaine/Ankles.mp4`;

// ===== מדידות גוף - שמות באנגלית =====
export const userSizes = [
  "Chest",
  "Waist",
  "Seat",
  "Jacket Length",
  "Front Width",
  "Rear Width",
  "Armhole",
  "Biceps",
  "Shoulder",
  "Sleeve Length",
  "Trousers Length",
  "Waistband",
  "Stride Length",
  "Thigh",
  "Knee",
  "Ankles",
];

// ===== מדידות חליפה - שמות באנגלית =====
export const userSizesSuit = [
  "Chest",
  "Waist",
  "Seat",
  "Jacket Length",
  "Front Width",
  "Rear Width",
  "Armhole",
  "Biceps",
  "Shoulder",
  "Sleeve Length",
  "Trousers Length",
  "Waistband",
  "Stride Length",
  "Thigh",
  "Knee",
  "Ankles",
];
// ===== נקודות על תמונה דו-ממדית למדידות גוף (למסך צד) =====
export const bodyPoints = [
  {
    id: "chest",
    label: "חזה",
    x: 60,
    y: 35,
    category: userSizes[0],
  },
  {
    id: "waist",
    label: "מותניים",
    x: 60,
    y: 50,
    category: userSizes[1],
  },
  {
    id: "bat",
    label: "טוסיק",
    x: 60,
    y: 65,
    category: userSizes[2],
  },
  {
    id: "hips",
    label: "אורך חליפה",
    x: 40,
    y: 25,
    category: userSizes[3],
  },
  {
    id: "shoulderWidth",
    label: "חזה עליון",
    x: 60,
    y: 30,
    category: userSizes[4],
  },
  {
    id: "sleeveLength",
    label: "אורך גב",
    x: 88,
    y: 32,
    category: userSizes[5],
  },
  {
    id: "bicep",
    label: "היקף זרוע",
    x: 70,
    y: 26,
    category: userSizes[6],
  },
  {
    id: "wrist",
    label: "היקף פרק כף יד",
    x: 72,
    y: 35,
    category: userSizes[7],
  },
  {
    id: "inseam",
    label: "אורך רגל פנימי",
    x: 88,
    y: 70,
    category: userSizes[8],
  },
  {
    id: "outseam",
    label: "אורך שרוול",
    x: 71,
    y: 62,
    category: userSizes[9],
  },
  {
    id: "thigh",
    label: "אורך מכנס",
    x: 42,
    y: 65,
    category: userSizes[10],
  },
  {
    id: "knee",
    label: "היקף מוטן",
    x: 51,
    y: 60,
    category: userSizes[11],
  },
  {
    id: "calf",
    label: "אורך רוחסן",
    x: 51,
    y: 67,
    category: userSizes[12],
  },
  {
    id: "ankle",
    label: "היקף קרסול",
    x: 43,
    y: 75,
    category: userSizes[13],
  },
  {
    id: "jacketLength",
    label: "אורך ז׳קט",
    x: 43,
    y: 85,
    category: userSizes[14],
  },
  {
    id: "trouser-length",
    label: "אורך מכנסיים",
    x: 88,
    y: 50,
    category: userSizes[15],
  },
];
export const sizesCollectionExpleines = [
  {
    img: size1,
    video: null,
    title:
      "לוקחים את המטר ושמים מסביב למוטן מעדקים ואז אחרי זה שמים אצבע אחד כדי שיהיה מקו",
  },
  { img: size2, video: null, title: "" },
  { img: size3, video: null, title: "" },
  { img: size4, video: null, title: "" },
  { img: size5, video: null, title: "" },
  { img: size6, video: null, title: "" },
  { img: size7, video: null, title: "" },
  { img: size8, video: null, title: "" },
  { img: size9, video: null, title: "" },
  { img: size10, video: null, title: "" },
  { img: size11, video: null, title: "" },
  { img: size12, video: null, title: "" },
  { img: size13, video: null, title: "" },
  { img: size14, video: null, title: "" },
  { img: size15, video: null, title: "" },
  { img: size16, video: null, title: "" },
];
// ===== נקודות על תמונה דו-ממדית למדידות חליפה (למסך צד) =====
export const bodyPointsSuit = [
  {
    id: "chest",
    label: "היקף חזה",
    x: 60,
    y: 35,
    category: userSizesSuit[0],
  },
  {
    id: "waist",
    label: "היקף מותניים",
    x: 60,
    y: 50,
    category: userSizesSuit[1],
  },
  {
    id: "seat",
    label: "היקף ירכיים",
    x: 60,
    y: 65,
    category: userSizesSuit[2],
  },
  {
    id: "jacketLength",
    label: "אורך ז'קט",
    x: 40,
    y: 25,
    category: userSizesSuit[3],
  },
  {
    id: "frontWidth",
    label: "רוחב חזית",
    x: 60,
    y: 30,
    category: userSizesSuit[4],
  },
  {
    id: "rearWidth",
    label: "רוחב גב",
    x: 88,
    y: 32,
    category: userSizesSuit[5],
  },
  {
    id: "armhole",
    label: "היקף בית שחי",
    x: 70,
    y: 26,
    category: userSizesSuit[6],
  },
  {
    id: "biceps",
    label: "היקף זרוע",
    x: 72,
    y: 35,
    category: userSizesSuit[7],
  },
  {
    id: "shoulder",
    label: "רוחב כתף",
    x: 88,
    y: 70,
    category: userSizesSuit[8],
  },
  {
    id: "sleeveLength",
    label: "אורך שרוול",
    x: 71,
    y: 62,
    category: userSizesSuit[9],
  },
  {
    id: "trousersLength",
    label: "אורך מכנסיים",
    x: 42,
    y: 65,
    category: userSizesSuit[10],
  },
  {
    id: "waistband",
    label: "היקף חגורה",
    x: 51,
    y: 60,
    category: userSizesSuit[11],
  },
  {
    id: "strideLength",
    label: "אורך צעד",
    x: 51,
    y: 67,
    category: userSizesSuit[12],
  },
  {
    id: "thigh",
    label: "היקף ירך",
    x: 43,
    y: 75,
    category: userSizesSuit[13],
  },
  {
    id: "knee",
    label: "היקף ברך",
    x: 43,
    y: 85,
    category: userSizesSuit[14],
  },
  {
    id: "ankles",
    label: "היקף קרסול",
    x: 88,
    y: 50,
    category: userSizesSuit[15],
  },
];

// ===== כפתורים על המודל התלת-ממדי למדידות גוף =====
export const buttons = [
  {
    id: 1,
    name: "Chest",
    position: [0, 6.2, 1],
    size: 0.15,
    tooltip: "חזה עליון",
    label: "חזה עליון",
    explain: `
      שלב 1:
      קח את סרט המדידה והנח אותו על החזה העליון
      (באזור שנמצא מעט מתחת לבית השחי, מעל החזה עצמו)

      שלב 2:
      סגור את המטר סביב החזה
      הכנס אצבע אחת מתחת לסרט המדידה
      כך תבטיח שיש מספיק מרווח לנשימה

      שלב 3:
      וודא שהמדידה נעשית בחלק העליון ביותר של החזה
      לא באמצע החזה ולא בחלקו התחתון
    `,
    img: size1,
    video: Chest,
  },
  {
    id: 2,
    name: "Waist",
    position: [0, 5.5, 1],
    size: 0.15,
    tooltip: "היקף מותניים",
    label: "היקף מותניים",
    explain: `
      שלב 1:
      קח את סרט המדידה
      מקם אותו בדיוק בגובה הטבור
      לא גבוה מדי ולא נמוך מדי

      שלב 2:
      הקף את הגוף עם סרט המדידה
      כמו שאתה חוגר חגורה
      וודא שהסרט מונח בצורה ישרה

      שלב 3:
      קרא את המספר שמופיע על הסרט
      זוהי מידת היקף המותניים שלך
    `,
    img: size2,
    video: Waist,
  },
  {
    id: 3,
    name: "Seat",
    position: [0, 4.6, -1],
    size: 0.15,
    tooltip: "היקף ירכיים",
    label: "היקף ירכיים",
    explain: `
      שלב 1:
      קח את סרט המדידה
      עמוד בצורה ישרה וטבעית

      שלב 2:
      הנח את הסרט מסביב לישבן
      מקם אותו בחלק הרחב ביותר
      
      שלב 3:
      וודא שהסרט מונח בדיוק על האזור הרחב ביותר
      כך תקבל מידה מדויקת שתהיה נוחה בישיבה
      ותאפשר תחושה טובה בלבישה
    `,
    img: size3,
    video: Seat,
  },
  {
    id: 4,
    name: "Jacket Length",
    position: [-0.7, 7.25, 1],
    size: 0.15,
    tooltip: "אורך ז'קט",
    label: "אורך ז'קט",
    explain: `
      שלב 1:
      קח את סרט המדידה
      עמוד זקוף עם ידיים משוחררות לצדדים

      שלב 2:
      מצא את הנקודה הגבוהה ביותר בכתף
      זו הנקודה שקרובה לצוואר
      הנח שם את תחילת סרט המדידה

      שלב 3:
      העבר את הסרט לאורך היד
      שמור על קו ישר
      המשך עד לנקודת העצם הבולטת
      ליד האגודל בפרק כף היד
    `,
    img: size4,
    video: JacketLength,
  },
  {
    id: 5,
    name: "Front Width",
    position: [0, 6.8, 1],
    size: 0.15,
    tooltip: "רוחב חזית",
    label: "רוחב חזית",
    explain: `
      שלב 1:
      עמוד בצורה זקופה וטבעית
      הורד את הידיים בנוחות לצדי הגוף

      שלב 2:
      אתר את החלק העליון של בית השחי
      זהו המקום בו היד מתחברת לגוף
      
      שלב 3:
      הנח את סרט המדידה בנקודה זו
      העבר אותו בקו ישר לצד השני
      עד לנקודה המקבילה בצד השני
      
      שלב 4:
      וודא שהסרט מונח בצורה ישרה
      ושהמדידה נעשית באותו גובה משני הצדדים
    `,
    img: size5,
    video: FrontWidth,
  },
  {
    id: 6,
    name: "Rear Width",
    position: [0, 6.5, -1],
    size: 0.15,
    tooltip: "רוחב גב",
    label: "רוחב גב",
    explain: `
      1. עמוד ישר.
      2. ידיים כלפי מטה לצדדים.
      3. קח את המטר והנח אותו בגובה החלק העליון של בית השחי מאחור, בדיוק במקום שבו היד מתחברת לגוף.
      4. סובב את המטר לצד השני, עד לנקודה זהה בחיבור של היד עם הגוף בצד השני.
    `,
    img: size6,
    video: RearWidth,
  },
  {
    id: 8,
    name: "Biceps",
    position: [1.3, 6.3, -0.2],
    size: 0.15,
    tooltip: "היקף זרוע",
    label: "היקף זרוע",
    explain: `
      1. עמוד זקוף עם ידיים כלפי מטה.
      2. קח את המטר והנח אותו על החלק הגדול ביותר בשריר של היד.
      3. שים אצבע אחת בין המטר ליד – כך שהמדידה תהיה רופפת וקלה, לא הדוקה מדי.
    `,
    img: size8,
    video: Biceps,
  },
  {
    id: 9,
    name: "Shoulder",
    position: [0, 7.5, -1],
    size: 0.15,
    tooltip: "רוחב כתף",
    label: "רוחב כתף",
    explain: `
      1. עמוד זקוף עם ידיים כלפי מטה בצדדים.
      2. מצא את חלק הכתף העליון, הנקודה העגולה שבה העצם מתחברת לגוף.
      3. הנח את קצה המטר בנקודה הזו בצד אחד.
      4. סובב את המטר בצורת קשת עגולה מעל הכתף, כך שהוא עוקף את הקימור הטבעי של הכתף ולא שוקע ישר על הגב.
      5. המשך את המטר עד לנקודת החיבור המקבילה בצד השני של הכתף.
    `,
    img: size9,
    video: Shoulder,
  },
  {
    id: 10,
    name: "Sleeve Length",
    position: [1.8, 4.8, 0],
    size: 0.15,
    tooltip: "אורך שרוול",
    label: "אורך שרוול",
    explain: `
      1. עמוד זקוף עם ידיים כלפי מטה בצדדים.
      2. מצא את נקודת החיבור של הכתף עם היד – זהו נקודת ההתחלה של המדידה.
      3. חלק ראשון:
         - הנח את המטר מהכתף ועד למרפק.
         - תן לאצבע לסמן את נקודת המרפק על המטר.
      4. חלק שני:
         - המשך את המטר מהאצבע ועד לסוף היד.
         - זהו המקום שבו תרצה שהשרוול של החליפה יגיע.
    `,
    img: size10,
    video: SleeveLengthFront,
  },
  {
    id: 11,
    name: "Trousers Length back",
    position: [0.7, 0.2, 1],
    size: 0.15,
    tooltip: "אחורי אורך מכנסיים",
    label: "אחורי אורך מכנסיים",
    explain: `
      1. עמוד זקוף עם מכנסי חליפה עדיפים, כשהמכנס יושב בנוחות על המותן והירך.
      2. קח את המטר והנח אותו על החלק העליון של המכנס מקדימה, מול הרגל, במקום שבו המכנס מתחיל מהמותן או המפשעה.
      3. מתח את המטר כלפי מטה לאורך הרגל עד העקב, כך שהמטר עוקב אחרי הרגל בצורה חלקה.
    `,
    img: size11,
    video: TrousersLengthFront,
  },
  {
    id: 12,
    name: "Trousers Length front",
    position: [0.6, 0.2, -0.6],
    size: 0.15,
    tooltip: "אורך מכנסים קדמי",
    label: "היקף ברך",
    explain: `
      1. עמוד זקוף עם מכנסי חליפה עדיפים, כשהמכנס יושב בנוחות על המותן והירך.
      2. קח את המטר והנח אותו בחלק העליון של המכנס מקדימה, מול הרגל – כלומר, במקום שבו המכנס מתחיל מהמותן או המפשעה קדימה.
      3. מתח את המטר כלפי מטה לאורך הרגל, עד סוף הבוהן.
    `,
    img: size15,
    video: TrousersLengthBack,
  },
  {
    id: 13,
    name: "Waistband",
    position: [0, 4.95, 1],
    size: 0.15,
    tooltip: "היקף חגורה",
    label: "היקף חגורה",
    explain: `
      1. עמוד זקוף עם מכנסי חליפה עדיפים.
      2. קח את המטר והנח אותו בדיוק מעל הסיומת של המכנס, על החלק שאתה רוצה למדוד (למשל המותן או הירך).
      3. המטר צריך להיות בגובה שבו המכנס מסתיים על הגוף, לא למטה ולא למעלה.
      4. ודא שהמטר מגע קל עם הגוף, אבל לא הדוק מדי – הוא צריך להיות בול רופף, מספיק מקום לתנועה.
    `,
    img: size12,
    video: Waistband,
  },
  {
    id: 14,
    name: "Stride Length",
    position: [0, 4.4, 1],
    size: 0.15,
    tooltip: "אורך צעד",
    label: "אורך צעד",
    explain: `
      1. עמוד זקוף עם מכנסי חליפה עדיפים.
      2. קח את המטר והנח אותו בנקודת ההתחלה של המכנס בצד אחד של הישבן.
      3. עבר עם המטר מתחת לישבן והבא אותו לנקודת ההתחלה בצד השני של המכנס.
      4. חלק ראשון – הדוק:
         - חבר את המטר בדיוק לנקודת ההתחלה משני הצדדים.
         - רשום את המידה בס"מ.
      5. חלק שני – רופף:
         - שחרר את המטר ב-1 אינץ' (2.5 ס"מ) כדי שיהיה רופף ונוח.
         - רשום את המידה הזו בס"מ.
    `,
    img: size13,
    video: StrideLength,
  },
  {
    id: 15,
    name: "Thigh",
    position: [0.5, 3.5, 1],
    size: 0.15,
    tooltip: "היקף ירך",
    label: "היקף ירך",
    explain: `
      1. עמוד זקוף עם מכנסי חליפה עדיפים.
      2. קח את המטר והנח אותו בחלק העליון ביותר של הרגל, ממש מתחת לישבן, באזור המפשעה.
      3. שים שלוש אצבעות בין המטר לגוף – כדי שיהיה רופף ויש מקום לתנועה.
    `,
    img: size14,
    video: Thigh,
  },
  {
    id: 16,
    name: "Ankles",
    position: [0.7, 0.5, 0.4],
    size: 0.15,
    tooltip: "היקף קרסול",
    label: "היקף קרסול",
    explain: `
      1. עמוד זקוף עם מכנסי חליפה עדיפים.
      2. קח את המטר והנח אותו בסוף הרגל, ממש באזור הקרסול.
      3. צור לולאה מסודרת עם המטר סביב הרגל – כמו עיגול קטן, כדי שהמכנס יירד יפה ולא יהיה הדוק מדי.
      4. התאמת המידה:
         - אם רוצים שהמכנס יהיה צמוד יותר – קח את המטר קצת יותר קרוב לרגל.
         - אם רוצים שהמכנס יהיה רחב יותר – תן למטר קצת יותר מקום סביב הרגל.
    `,
    img: size16,
    video: Ankles,
  },
];

// ===== כפתורים על המודל התלת-ממדי למדידות חליפה =====
export const buttonsSuit = [
  {
    id: 1,
    name: "Chest",
    position: [0, 4.2, 1.5],
    size: 0.15,
    tooltip: "היקף חזה",
    label: "היקף חזה",
    explain: "מדידת היקף החזה של החליפה - מסביב לחלק הרחב ביותר של החזה בחליפה",
    img: size1,
    video: Chest,
  },
  {
    id: 2,
    name: "Waist",
    position: [0, 5.3, 1.5],
    size: 0.15,
    tooltip: "היקף מותניים",
    label: "היקף מותניים",
    explain:
      "מדידת היקף המותניים של החליפה - מסביב לחלק הצר ביותר של המותניים בחליפה",
    img: size2,
    video: "",
  },
  {
    id: 3,
    name: "Seat",
    position: [-1.3, 6.6, 1.5],
    size: 0.15,
    tooltip: "היקף ירכיים",
    label: "היקף ירכיים",
    explain:
      "מדידת היקף הירכיים של החליפה - מסביב לחלק הרחב ביותר של הירכיים בחליפה",
    img: size3,
    video: "",
  },
  {
    id: 4,
    name: "Jacket Length",
    position: [1.3, 6.6, 1.5],
    size: 0.15,
    tooltip: "אורך ז'קט",
    label: "אורך ז'קט",
    explain: "מדידת אורך הז'קט - מהכתף עד לקצה הז'קט (לא כולל השרוולים)",
    img: size4,
    video: "",
  },
  {
    id: 5,
    name: "Front Width",
    position: [1.1, 5, 1.5],
    size: 0.15,
    tooltip: "רוחב חזית",
    label: "רוחב חזית",
    explain: "מדידת רוחב חזית הז'קט - הרוחב של החלק הקדמי של הז'קט",
    img: size5,
    video: "",
  },
  {
    id: 6,
    name: "Rear Width",
    position: [-0.7, 2.1, 1.5],
    size: 0.15,
    tooltip: "רוחב גב",
    label: "רוחב גב",
    explain: "מדידת רוחב גב הז'קט - הרוחב של החלק האחורי של הז'קט",
    img: size6,
    video: "",
  },
  {
    id: 7,
    name: "Armhole",
    position: [2, 7.5, -0.4],
    size: 0.15,
    tooltip: "היקף בית שחי",
    label: "היקף בית שחי",
    explain: "מדידת היקף בית השחי של הז'קט - היקף הפתח של השרוול בז'קט",
    img: size7,
    video: "",
  },
  {
    id: 8,
    name: "Biceps",
    position: [3.3, 6.3, -0.3],
    size: 0.15,
    tooltip: "היקף זרוע",
    label: "היקף זרוע",
    explain: "מדידת היקף הזרוע בחליפה - בחלק הרחב ביותר של הזרוע בחליפה",
    img: size8,
    video: "",
  },
  {
    id: 9,
    name: "Shoulder",
    position: [5, 5, 0.5],
    size: 0.15,
    tooltip: "רוחב כתף",
    label: "רוחב כתף",
    explain: "מדידת רוחב הכתף בחליפה - מכתף לכתף על הז'קט",
    img: size9,
    video: "",
  },
  {
    id: 10,
    name: "Sleeve Length",
    position: [0, 7.8, -1.5],
    size: 0.15,
    tooltip: "אורך שרוול",
    label: "אורך שרוול",
    explain: "מדידת אורך השרוול - מהכתף עד לקצה השרוול של הז'קט",
    img: size10,
    video: "",
  },
  {
    id: 11,
    name: "Trousers Length",
    position: [-2, 7.5, -0.4],
    size: 0.15,
    tooltip: "אורך מכנסיים",
    label: "אורך מכנסיים",
    explain: "מדידת אורך המכנסיים - מהמותניים עד לקצה המכנסיים",
    img: size11,
    video: "",
  },
  {
    id: 12,
    name: "Waistband",
    position: [0, 4.8, 3],
    size: 0.15,
    tooltip: "היקף חגורה",
    label: "היקף חגורה",
    explain: "מדידת היקף החגורה של המכנסיים",
    img: size12,
    video: "",
  },
  {
    id: 13,
    name: "Stride Length",
    position: [0, 4.4, 3],
    size: 0.15,
    tooltip: "אורך צעד",
    label: "אורך צעד",
    explain: "מדידת אורך הצעד - מהמותניים עד לרצפה דרך המפשעה",
    img: size13,
    video: "",
  },
  {
    id: 14,
    name: "Thigh",
    position: [0.5, 3.5, 3],
    size: 0.15,
    tooltip: "היקף ירך",
    label: "היקף ירך",
    explain: "מדידת היקף הירך - בחלק הרחב ביותר",
    img: size14,
    video: "",
  },
  {
    id: 15,
    name: "Knee",
    position: [0.5, 3, 3],
    size: 0.15,
    tooltip: "היקף ברך",
    label: "היקף ברך",
    explain: "מדידת היקף הברך",
    img: size15,
    video: "",
  },
  {
    id: 16,
    name: "Ankles",
    position: [0.7, 0.4, 3],
    size: 0.15,
    tooltip: "היקף קרסול",
    label: "היקף קרסול",
    explain: "מדידת היקף הקרסול",
    img: size16,
    video: "",
  },
];

export const collections = [
  { id: 1, title: "Classic Elegance", image: Suit1 },
  { id: 2, title: "Modern Sophistication", image: Suit2 },
  { id: 3, title: "Italian Heritage", image: Suit3 },
  { id: 4, title: "Luxury Essentials", image: Suit4 },
  { id: 5, title: "Timeless Style", image: Suit5 },
  { id: 6, title: "Exclusive Collection", image: Suit6 },
  { id: 7, title: "Tailored Perfection", image: Suit7 },
  { id: 8, title: "Prestige Line", image: Suit8 },
  { id: 9, title: "Royal Elegance", image: Suit9 },
  { id: 10, title: "Urban Gentleman", image: Suit10 },
  { id: 11, title: "Sharp & Sleek", image: Suit11 },
  { id: 12, title: "Modern Classic", image: Suit12 },
  { id: 13, title: "Black Tie Affair", image: Suit13 },
  { id: 14, title: "Executive Style", image: Suit14 },
  { id: 15, title: "Bespoke Beauty", image: Suit15 },
  { id: 16, title: "Chic & Sharp", image: Suit16 },
  { id: 17, title: "Contemporary Charm", image: Suit17 },
  { id: 18, title: "Sophisticated Silhouette", image: Suit18 },
  { id: 19, title: "Elite Elegance", image: Suit19 },
  { id: 20, title: "Refined Gentleman", image: Suit20 },
  { id: 21, title: "Dapper Choice", image: Suit21 },
];

export const imagesInsideUp = [
  { name: "blueCube", img: blueCube },
  { name: "blueLight", img: blueLight },
  { name: "blueWithWhiteLines", img: blueWithWhiteLines },
  { name: "colorFull", img: colorFull },
  { name: "gold", img: gold },
  { name: "grey", img: grey },
  { name: "purple", img: purple },
  { name: "redPurple", img: redPurple },
  { name: "whiteBlue", img: whiteBlue },
];

export const imagesHoles = [
  { name: "black", color: "#000000" },
  { name: "white", color: "#FFFFFF" },
  { name: "blue", color: "#0000FF" },
  { name: "blueLight", color: "#ADD8E6" },
  { name: "grey", color: "#808080" },
  { name: "pruple", color: "#800080" },
  { name: "brown", color: "#A52A2A" },
  { name: "pink", color: "#FFC0CB" },
  { name: "whiteDark", color: "#FFFACD" },
  { name: "red", color: "#FF0000" },
  { name: "orange", color: "#B67B6B" },
];

export const imagesHolesUp = [
  { name: "black", color: "#000000" },
  { name: "white", color: "#FFFFFF" },
  { name: "blue", color: "#0000FF" },
  { name: "blueLight", color: "#ADD8E6" },
  { name: "grey", color: "#808080" },
  { name: "pruple", color: "#800080" },
  { name: "brown", color: "#A52A2A" },
  { name: "pink", color: "#FFC0CB" },
  { name: "whiteDark", color: "#FFFACD" },
  { name: "red", color: "#FF0000" },
  { name: "orange", color: "#B67B6B" },
];

// export const imagesPoshet = [
//   { name: "white", color: "#FFFFFF" },
//   { name: "black", color: "#000000" },
//   { name: "blueLight", color: "#ADD8E6" },
//   { name: "blue", color: "#87CEEB" },
//   { name: "grey", color: "#808080" },
//   { name: "red", color: "#FF0000" },
//   { name: "pink", color: "#FFC0CB" },
//   { name: "yellow", color: "#FFFF00" },
// ];
export const imagesPoshet = [
  // { name: "black", img: poset1 },
  { name: "black", img: poset2 },
  // { name: "black3", img: poset3 },
  { name: "blackWithDots", img: poset4 },
  // { name: "blue", img: poset5 },
  { name: "blueLight", img: poset6 },
  { name: "blueLightWithDots", img: poset7 },
  // { name: "blueNevey", img: poset8 },
  { name: "blue", img: poset9 },
  { name: "blueWithBlueLightLins", img: poset10 },
  { name: "blueWithBlueLightLins2", img: poset11 },
  { name: "blueWithDots", img: poset12 },
  { name: "blueWithDots2", img: poset13 },
  { name: "blueWithLines", img: poset14 },
  { name: "brown", img: poset15 },
  { name: "brownWithDots", img: poset16 },
  { name: "gold", img: poset17 },
  { name: "goldLight", img: poset18 },
  { name: "grey", img: poset19 },
  { name: "peenk", img: poset20 },
  { name: "prupleWithDots", img: poset21 },
  { name: "purple", img: poset22 },
  { name: "purpleLight", img: poset23 },
  // { name: "red", img: poset24 },
  { name: "red", img: poset25 },
  { name: "redLight", img: poset26 },
  // { name: "redWithDots", img: poset27 },
  { name: "redWithDots2", img: poset28 },
  { name: "redWithines", img: poset29 },
  { name: "white", img: poset30 },
];

export const imageButton = [
  { name: "blackBlue", img: blackBlue },
  { name: "brown", img: brownBlack },
  { name: "brownWhite", img: brownWhite },
  { name: "darkGrey", img: greyWhite },
  { name: "gold", img: gold2 },
  { name: "goldLight", img: goldLight },
  { name: "greyBlack", img: greyBlack },
  { name: "silver", img: silver },
  { name: "silverLight", img: silverLight },
  { name: "white", img: white },
];

export const suitsColors = [
  {
    color: photo1,
    colorId: 1,
    colorName: "greyLight",
    name: "greyLight",
    price: "200$",
  },
  {
    color: photo2,
    colorId: 2,
    colorName: "black",
    name: "black",
    price: "250$",
  },
  {
    color: photo3,
    colorId: 3,
    colorName: "TR224-18",
    name: "blueLight",
    price: "200$",
    hexCode: "#B4B4B4",
  },
  {
    color: photo4,
    colorId: 4,
    colorName: "TR224-8",
    name: "brown",
    price: "200$",
    hexCode: "#DC143C",
  },
  {
    color: photo5,
    colorId: 5,
    colorName: "TR224-2",
    name: "brownlight",
    price: "200$",
    hexCode: "#50C878",
  },
  {
    color: photo6,
    colorId: 6,
    colorName: "(TR224-11) burgundy",
    name: "burgundy",
    price: "200$",
    hexCode: "#50C878",
  },
  {
    color: photo7,
    colorId: 7,
    colorName: "TR224-1",
    name: "beze",
    price: "200$",
    hexCode: "#50C878",
  },
  {
    color: photo8,
    colorId: 8,
    colorName: "TR224-17",
    name: "navy",
    price: "200$",
    hexCode: "#50C878",
  },
  {
    color: photo9,
    colorId: 8,
    colorName: "TR224-17",
    name: "navy",
    price: "200$",
    hexCode: "#50C878",
  },
];
